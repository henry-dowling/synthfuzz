...
if __name__ == "__main__":
    # Create a test signal (similar to the one in the notebook)
    duration = 10
    sample_rate = 44100
    time = np.linspace(0, duration, duration * sample_rate)
    test_signal = (2 * np.sin(2*np.pi*100*time) + 
                  3 * np.sin(2*np.pi*200*time) + 
                  4 * np.sin(2*np.pi*300*time))
    ...
    main(test_signal, 
         ...
         window_size=10000,
         plot_length=1000,
         plot_offset=int(2.5 * 44100),  # Start plotting at 2.5 seconds
